 ...
 */
package com.example.ammbr.Modelo.Negocio;
...
public class Distancia {
    ...
    public String MenorDistancia(double lat_pa, double long_pa) {
        CRUDSitioHospedaje sitioDao = new CRUDSitioHospedaje(); //atributo cuentabancoDAo es de tipo cuentabancoDao e inicia como nuevo cuentabancodaoimpl
        List<SitioHospedaje> list = sitioDao.listarSitiosHospedaje();
        String IDmenor = "";
        int size = list.size();
        int k=0;
        double array[] = new double [size];
        double menor;
        System.out.println("HASTA AQUI NO HAY ERRPRES: ");
        for (SitioHospedaje SitioHospedaje : list) {
            double lat_des = Double.parseDouble(SitioHospedaje.getLatitud());
            double long_des = Double.parseDouble(SitioHospedaje.getLongitud());
        ...
        IDmenor = list.get(menorIndex).getIdHospedaje();
        return IDmenor;
    }
