...
public class HabitacionDaoImpl implements HabitacionDao{
    ...
    public List<Habitacion> list() {
        ...
        try{
            ...
            while (rs.next()){  //mientras haya registros en la tabla
                ...
                Habitacion.setIdHabitacion(rs.getLong("IDHABITACION"));
    ...
    @Override
    public List<Habitacion> listarSitioHospedaje(String idHospedaje) {
        this.conn = FactoryConexionDB.open();

        StringBuilder sql = new StringBuilder();
        sql.append("SELECT * FROM habitacion WHERE IDHOSPEDAJE = '").append(idHospedaje);  //construye la cadena de consulta
        sql.append("'");
        ...
        try{
            ...
            while (rs.next()){  //mientras haya registros en la tabla
                Habitacion Habitacion = new Habitacion();
                Habitacion.setIdHabitacion(rs.getLong("IDHABITACION"));
                Habitacion.setCategoriaHabitacion(rs.getString("CATEGORIAHABITACION"));
                Habitacion.setIdHospedaje(rs.getString("IDHOSPEDAJE"));
                Habitacion.setNumeroCamas(rs.getInt("NUMEROCAMAS"));
                Habitacion.setDescripcion(rs.getString("DESCRIPCION"));
                Habitacion.setPrecio(rs.getDouble("PRECIO"));
            ...
            System.out.println("Error en lista de habitaciones: " + e);
        } finally {
            this.conn.close();      //cierra la conexion
        }
        return list;    //devuelve la lista generada
    ...
    public boolean save(Habitacion Habitacion) {
        ...
        try {
            ...
            if (find == false) {
                ...
                sql.append("INSERT INTO habitacion (CategoriaHabitacion, IDHOSPEDAJE, NUMEROCAMAS, DESCRIPCION, "
                        + "PRECIO) VALUES ('").append(Habitacion.getCategoriaHabitacion());
    ...
    public boolean ExisteHabitacion(String idHospedaje) {
        ...
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT * FROM habitacion WHERE IDHOSPEDAJE = '").append(idHospedaje);
        sql.append("'");
